...
let astronautsInfoURL = "http://api.open-notify.org/astros.json"  // 우주비행사 정보
let apiURL = URL(string: astronautsInfoURL)!
...
let dataTask = URLSession.shared.dataTask(with: apiURL) { (data, response, error) in
  ...
  if let astronautsJSONData = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
    guard
      let people = astronautsJSONData["people"] as? [[String: String]]
      else { return }

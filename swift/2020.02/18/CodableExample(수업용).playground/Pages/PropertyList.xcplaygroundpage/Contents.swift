...
let macBookArr = [macBook, macBook, macBook]
let macBookDict = ["mac": macBook, "mac1": macBook, "mac2": macBook]

if let decodedMacBookArr = try? pListEncoder.encode(macBookArr) {
  print(decodedMacBookArr)
  
  if let encodedMacBookArr = try? pListDecoder.decode([MacBook].self, from: decodedMacBookArr) {
    print(encodedMacBookArr)
  }
}

if let decodedMacBookDict = try? pListEncoder.encode(macBookDict) {
  print(decodedMacBookDict)
  
  if let encodedMacBookDict = try? pListDecoder.decode([String: MacBook].self, from: decodedMacBookDict) {
    print(encodedMacBookDict)
  }
}

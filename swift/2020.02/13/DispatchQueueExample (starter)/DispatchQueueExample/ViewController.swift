...
//    queue2.async { calculate(task: 3, limit:  2_000_000) }
    ...
    group.notify(queue: .main) { print("has finished all the tasks") }
  ...
  @IBAction func cancelDispatchWorkItem() {
    ...
    DispatchQueue.global().async {
      let timeourResult = self.myWorkItem.wait(timeout: .now() + timeLimit)
      switch timeourResult {
      case .success:

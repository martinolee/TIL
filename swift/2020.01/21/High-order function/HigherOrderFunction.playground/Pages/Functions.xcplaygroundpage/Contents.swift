...
func printOnlyEvenNumberAndLessThenTen(_ num: Int) {
  if (num < 10) && (num % 2 == 0) {
    print(num, terminator: " ")
  }
}

oneToTen.forEach(printOnlyEvenNumberAndLessThenTen(_:))
print("")
...
print(type(of: ["1", "2", "3"].reduce("", +)))
["1", "2", "3"].reduce("", +)

print(type(of: [1, 2, 3].reduce("") { $0 + String($1) }))
[1, 2, 3].reduce("") { $0 + String($1) }
